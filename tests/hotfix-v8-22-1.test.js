...
const central=fs.readFileSync(path.join(root,'central-modules-hotfix-v8-22-1.js'),'utf8');
...
test('PWA injeta módulos centrais e recuperação mesmo sobre shell antigo',()=>{
  assert.match(sw,/john-erp-pwa-v8\.22\.1-central-hotfix/);
  ...
  assert.match(sw,/central-modules-hotfix-v8-22-1\.js/);
});

test('Motor Comercial e Central Gerencial são recriados como grupos centrais',()=>{
