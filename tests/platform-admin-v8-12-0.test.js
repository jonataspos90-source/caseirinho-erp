...
test('V8.14 mantém namespace por tenant',()=>{
...
test('service worker V8.14 preserva multiempresa e painel consolidado',()=>{
  assert.match(sw,/multiempresa-v8-12-0\.js\?v=8140/);
  assert.match(sw,/platform-admin-v8-12-0\.js\?v=8140/);
...
test('cliente não recebe tela pública de token de ativação',()=>{
  assert.doesNotMatch(multi,/Primeiro acesso da empresa/);
  assert.doesNotMatch(multi,/johnBootToken/);
  assert.doesNotMatch(multi,/Ativar empresa e criar MASTER/);
  assert.match(multi,/Empresa ainda não liberada/);
});

test('URL sem empresa não reutiliza a última empresa visitada',()=>{
  assert.match(multi,/const tenantSlug=fromUrl\|\|'caseirinho'/);
  assert.doesNotMatch(multi,/fromUrl\|\|remembered\|\|legacySlug/);
});

test('manifesto ERP preserva tenant no start_url',()=>{
  assert.match(multi,/function applyTenantManifest/);
  assert.match(multi,/start_url:`\.\/\?empresa=/);
});

test('painel exibe links exclusivos de ERP e Loja',()=>{
  assert.match(panel,/ERP_PUBLIC_BASE/);
  assert.match(panel,/STORE_PUBLIC_BASE/);
  assert.match(panel,/Copiar ERP/);
  assert.match(panel,/Copiar Loja/);
  assert.match(panel,/john-'\+base/);
});

test('painel reexecuta descoberta após login da sessão',()=>{
  assert.match(panel,/john:session-ready/);
  assert.match(multi,/john:session-ready/);
});


test('login não permite trocar empresa manualmente ou ativar tenant por token',()=>{
  assert.match(multi,/link exclusivo/);
  assert.doesNotMatch(multi,/johnChangeTenant/);
  assert.doesNotMatch(multi,/createMaster/);
  assert.doesNotMatch(multi,/bootstrap\/master/);
});
