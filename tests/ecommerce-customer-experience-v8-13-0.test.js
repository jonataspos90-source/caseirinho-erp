...
test('service worker injeta módulos consolidados no clean rebuild atual',()=>{assert.match(sw,/platform-admin-v8-12-0\.js/);assert.match(sw,/ecommerce-customer-experience-v8-13-0\.js/);assert.match(sw,/const CACHE='john-erp-pwa-v8\.22\.\d+[-\w]*'/);assert.match(sw,/server-catalog-hydration-v8-19\.js/);assert.match(sw,/transaction-persistence-v8-22-12\.js/)});
