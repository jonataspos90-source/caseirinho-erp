...
['pointerdown','touchstart','wheel','keydown'].forEach(ev=>window.addEventListener(ev,()=>{userInteractionSeq++},{passive:true,capture:true}));
  ...
  try{
    ...
    for(const p of pedidos){
      if(p&&p.pagamentoIntegral===true){
        p.restante=0;
        const total=Number(p.valorTotal??p.total??p.subtotal??0)||0;
        if(!(Number(p.valorPagoDireto)>0)&&total>0)p.valorPagoDireto=total;
      }
    }
  }catch(_){ }
}
...
function isSafeChangeGet(req){
  ...
  try{
    ...
    return /\/api\/v1\/admin\/store\/orders\/changes$/.test(u.pathname)||/\/api\/v1\/admin\/sync\/changes$/.test(u.pathname);
  }catch(_){return false}
}
function isOrdersChange(req){try{return /\/api\/v1\/admin\/store\/orders\/changes$/.test(new URL(req.url,location.href).pathname)}catch(_){return false}}
window.fetch=async function(input,init){
  let req;try{req=new Request(input,init)}catch(_){return nativeFetch(input,init)}
  if(!isSafeChangeGet(req))return nativeFetch(input,init);
  ...
  if(isOrdersChange(req)){
    setTimeout(()=>sleepFrame(()=>{
      if(userInteractionSeq===interactionAtStart && Math.abs(window.scrollY-yAtStart)>2)window.scrollTo(xAtStart,yAtStart);
...
function addCss(){if(cssApplied)return;cssApplied=true;const st=document.createElement('style');st.id='john-prod-stability-8105-css';st.textContent=`
  body{overflow-anchor:none}
  [data-john-orders-stable="1"]{table-layout:fixed!important;width:100%!important}
  [data-john-orders-stable="1"] th,[data-john-orders-stable="1"] td{vertical-align:middle!important}
  [data-john-orders-stable="1"] th:last-child,[data-john-orders-stable="1"] td:last-child{min-width:250px!important}
  [data-john-orders-stable="1"] td:last-child button,[data-john-orders-stable="1"] td:last-child .btn{white-space:nowrap!important}
  @media(max-width:760px){
    [data-john-orders-wrap="1"]{overflow-x:auto!important;-webkit-overflow-scrolling:touch;overscroll-behavior-x:contain}
...
function stabilizeOrdersTable(){
  ...
  for(const t of tables){
    ...
    if(/PEDIDO/i.test(text)&&/STATUS/i.test(text)&&/AÇÕES|ACOES/i.test(text)){
      ...
      const wrap=t.parentElement;if(wrap)wrap.dataset.johnOrdersWrap='1';
    }
  }
}
...
console.info('[John ERP] estabilidade V8.10.5 ativa');
})();
