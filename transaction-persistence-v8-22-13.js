(function(){
'use strict';
if(window.__JOHN_TX_PERSIST_82213__)return;
window.__JOHN_TX_PERSIST_82213__=true;
...
function mergeOrders(local,remote){const out=clone(A(local));for(const r of A(remote)){const ix=out.findIndex(x=>sameOrder(x,r));if(ix<0)out.push(clone(r));else if(ts(r)>=ts(out[ix]))out[ix]=clone(r)}return out.sort((a,b)=>(Number(a?.numero)||0)-(Number(b?.numero)||0))}
...
function mergePeople(local,remote){const out=clone(A(local));for(const r of A(remote)){const k=personKey(r),ix=k?out.findIndex(x=>personKey(x)===k):-1;if(ix<0)out.push(clone(r));else if(ts(r)>=ts(out[ix]))out[ix]=clone(r)}return out}
function snapshotFromDb(d){if(!d||typeof d!=='object')return null;return{version:VERSION,capturedAt:new Date().toISOString(),pedidos:clone(A(d.pedidos)),pessoas:clone(A(d.pessoas)),convenioDuplicatas:clone(A(d.convenioDuplicatas)),convenioPagamentos:clone(A(d.convenioPagamentos)),fluxoCaixa:clone(A(d.fluxoCaixa)),vendas:clone(A(d.vendas))}}
function mergeSnapshot(primary,secondary){if(!primary)return secondary?clone(secondary):null;if(!secondary)return clone(primary);const out=clone(primary);out.pedidos=mergeOrders(primary.pedidos,secondary.pedidos);out.pessoas=mergePeople(primary.pessoas,secondary.pessoas);for(const k of ['convenioDuplicatas','convenioPagamentos','fluxoCaixa','vendas']){const a=A(out[k]);const seen=new Set(a.map(x=>S(x?.id)).filter(Boolean));for(const x of A(secondary[k])){const id=S(x?.id);if(!id||!seen.has(id)){a.push(clone(x));if(id)seen.add(id)}}out[k]=a}return out}
function buildBootBackup(){const existing=read(BACKUP_KEY),pre=read(PRE_ACTIVATION_KEY),current=read(DB_KEY);let snap=existing?clone(existing):null;if(!snap&&pre)snap=snapshotFromDb(pre);if(!snap&&current)snap=snapshotFromDb(current);if(snap&&pre)snap=mergeSnapshot(snap,snapshotFromDb(pre));if(snap&&current)snap=mergeSnapshot(snap,snapshotFromDb(current));if(snap)write(BACKUP_KEY,snap);return snap}
