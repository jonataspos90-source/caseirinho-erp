...
function saveLocal(d){try{const key=typeof DB_KEY!=='undefined'?DB_KEY:'pcp_app_v1';(typeof __johnLocalStorage!=='undefined'?__johnLocalStorage:localStorage).setItem(key,JSON.stringify(d))}catch(e){console.warn('[John V8.22.5] persistência local:',e)}}
...
function canonicalizeProduct(p,cp,cats){
 ...
 const imgs=[...new Set([cp?.imagem,...A(cp?.imagens)].map(S).filter(x=>x&& !/^blob:/i.test(x)))].slice(0,5);
 e.publicar=true;
...
function mergeCatalog(d,cat){
 ...
 const previousCats=A(d.config.ecommerce.categorias);
 const geral=previousCats.find(c=>S(c?.id).toUpperCase()==='GERAL'||c?.geral===true)||{id:'GERAL',nome:'Geral',geral:true,ativo:false,ordem:0,emoji:'✨'};
 const serverCats=A(cat?.loja?.categorias).map(normalizeCategory).filter(c=>c.id&&c.nome&&S(c.id).toUpperCase()!=='GERAL');
 d.config.ecommerce.categorias=[geral,...serverCats];
 ...
 return{products,images,unpublished,categoryCount:serverCats.length};
}
...
async function fetchCatalog(){const c=config();const r=await fetch(c.apiUrl+'/api/v1/public/store/'+encodeURIComponent(c.slug)+'/catalog?_john8225='+Date.now(),{cache:'no-store',headers:{'Cache-Control':'no-store','Pragma':'no-cache'}});let j={};try{j=await r.json()}catch(_){}if(!r.ok)throw new Error(j?.error||('HTTP '+r.status));return j}
...
window.johnHydrateStorefrontFromServerV8225=hydrate;
function schedule(force=false,ms=250){setTimeout(()=>hydrate(force).catch(e=>console.warn('[John V8.22.5] hidratação do catálogo:',e)),ms)}
