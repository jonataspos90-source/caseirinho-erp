...
function renderHub(){
 ...
 E('jnExpandAll')?.addEventListener('click',()=>{const cards=[...hub.querySelectorAll('.jn-module-card')],open=cards.length>0&&cards.every(c=>c.classList.contains('open'));cards.forEach(c=>c.classList.toggle('open',!open));syncExpandLabel()});
 syncExpandLabel();
 return true;
}
